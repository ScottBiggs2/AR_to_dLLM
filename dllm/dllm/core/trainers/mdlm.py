...
from dllm.core.masking import MaskingCurriculum, apply_curriculum_masking
import random
...
class MDLMTrainer(transformers.Trainer):
    ...
    def compute_loss(
        ...
        **kwargs,
        ...
        strategy = self.args.masking_strategy
        if strategy == "curriculum":
            probs = self.masking_curriculum.get_strategy_probs(self.state.global_step)
            strategy = random.choices(list(probs.keys()), weights=list(probs.values()))[0]
            
        noised_input_ids, masked_mask = apply_curriculum_masking(
            input_ids, labels, self.processing_class.mask_token_id, p_mask, strategy
        )
